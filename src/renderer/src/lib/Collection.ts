...
export default class Collection {
      ...
      const searchKeywords = search.request
        ...
        .map((keyword) =>
          keyword
            ...
            .split(" ")
            .map((keyword) =>
              keyword.split("　").map((keyword) => keyword.split("、")),
            ),
        )
        .flat(3)
        .map((keyword) =>
          toHiragana(keyword, { passRomaji: true }).toLowerCase(),
        );
          ...
          return properties.some((property) => {
            ...
            if (typeof property === "object") {
              return property.some((item) =>
                toHiragana(item, { passRomaji: true })
                  .toLowerCase()
                  .includes(keyword),
              );
            } else {
              return toHiragana(property, { passRomaji: true })
                .toLowerCase()
                .includes(keyword);
            }
          });
  ...
  private async parseWork(
    workDirectory: Dirent,
    userDirectory: Dirent,
    ...
    const assetsWithMetaFile: WorkAsset[] = rawAssetsWithMetaFile.map(
      (rawAsset) => ({
        name: rawAsset.name,
    ...
    const workData = metaFile
      ? await this.getWorkDataFromMetaFile(metaFile.path)
      : {};
